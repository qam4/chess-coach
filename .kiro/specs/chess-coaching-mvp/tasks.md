...
- [x] 9. Documentation and final polish
  - [x] 9.1 Set up GitHub Actions CI with pytest, ruff, and mypy (run on push to main and PRs)
  - [x] 9.2 Write `README.md` with installation, quickstart (CLI and web UI), configuration reference, and recommended models
  - [x] 9.3 Add docstrings to all public classes and functions
  - [x] 9.4 Run full test suite, mypy, ruff format, ruff check — all clean
  - [x] 9.5 Verify end-to-end: `chess-coach check`, `chess-coach explain <FEN>`, and `chess-coach serve` with a real engine and Ollama
