...
**Open actions: 10.** Generated — edit the fields under each heading, not this table.
...
### OPEN — Repeat each arm, because 60% of the coach's text is not reproducible

- **Category:** instrumentation
- **Gives:** the ability to tell a real change from noise. Without it no P1 above can be
  accepted or rejected. Measured: 32 of 81 spoken turns byte-identical across two runs of the
  same seed, same model, temperature 0; mean character similarity 0.70-0.83.
- **Priority:** P1

v53 and v54 ran the same five seeds on qwen3:14b with identical prompts. Every counter computed
from our own prompt was identical — `spoke`, `plies`, `cause_given_pct`, `lesson_conc_pct`,
`lesson_open_pct`. Every counter computed from the model's TEXT moved, in both directions:
`cause_voiced` -8pp on seed 7 and +5pp on seed 23, `clean_pct` +6pp on seed 17.

The seed 17 gain is the cautionary case. It is one turn — ply 65, the same turn whose "cut off
the opponent's king" justified the confinement check. **Its prompt was byte-identical in both
runs and the model simply said something else.** A one-turn difference is inside the churn.

So: prompt-side metrics need one run per arm and model-side metrics need several. What is
missing is the number of repeats, which has never been computed. The 20-run matrix gives a
cross-opening spread; this gives a same-cell spread; neither has been turned into "N runs to
detect X points". Do that first, then decide the run budget.
