...
Every actionable item — a heading starting `TOP —` or `OPEN —` — carries three fields directly
under its heading:
...
item cannot be added without saying what it gives.

<!-- BEGIN BACKLOG INDEX (generated by scripts/backlog_index.py - do not edit by hand) -->

**Open actions: 11.** Generated — edit the fields under each heading, not this table.
...
| P2 | decision | Two band questions that need judgement, not arithmetic | unknown until the band is chosen. It decides how many of ~18 spoken turns per game are about a move that is not actually bad, so it is upstream of the coaching, but no number can be attached without picking the band first. |
