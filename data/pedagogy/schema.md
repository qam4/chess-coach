...
| `excludes_features` | no | Position_Feature names whose presence makes this entry **wrong**, not merely irrelevant — the entry is then never selected, by any path. Validated against the same closed vocabulary, so a typo is rejected rather than silently never firing. Use sparingly: only where a principle *inverts*. The case it exists for is `principle.exposed_king`, which excludes `phase:endgame` because "get your exposed king to safety" is sound in the opening and middlegame and actively harmful in an endgame, where centralising the king is the winning method. Note `features` cannot express this: they are ANDed, so keying the entry to a phase would confine it to one phase and force the text to be duplicated per phase. |
