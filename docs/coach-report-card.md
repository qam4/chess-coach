...
| 77 | **v36 — the check landed, the score did not move, and the gating class is now precisely located.** Written before the run: ply 60 should be caught and, if it was the only gating fabrication, 2/10 -> ~5 | Nothing new: v35's coach plus the attack-geometry check | **Half right.** Ply 60 is fixed in production — it now reads "Rxh7 was stronger here — capturing their pawn on h7", true and composed. But the fidelity gate still fires (Fidelity 4) and the score is still **2/10, uncapped 5.1** — flat against v35's 5.1. Three new falsehoods, and they are all the same shape: ply 58 "Re4 hits the h7 pawn" (h7 is on neither rank 4 nor the e-file), ply 52 "Rh3+ wins control of the e-file", ply 38 "a3 prevents the opponent from targeting g2" | kept |
| 78 | (v36, and it reframes row 76) The judge located the defect for us, unprompted: *"every claim about what is hanging and what captures it checks out. The falsehoods live entirely in the clauses explaining why the recommended move is good. That is a bounded defect with a cheap fix, not a broken pipeline."* | **next** — the structural change already on the backlog, now with a target: the model must not author the "why" for a recommended move | — | This is the answer to "are we on an endless treadmill". **We are not.** Six checks each closed a different class because we were chasing instances; the classes were never independent. Every surviving fabrication sits in ONE slot — the justification clause for our own recommended move — and that is the slot we already compose when we can. The model appends invented reasons BEYOND what we supply. Material, hanging pieces and captures all verify clean. So the fix is not a seventh detector, it is withholding authorship of one clause | open — TOP |

| 79 | **v37 — the gate cleared for the first time in ten runs.** Written before the run: fidelity clears the gate, 2/10 -> ~5.1 | Model no longer authors the "why": the clause we composed is the only reason it may give, and where we have none it names the move with no reason. Backed by `_check_our_move_claims`, which pushes the recommended move and verifies attack and file-control claims. `_ATTACK_TARGET_RE` now reads both word orders | **Right, and slightly better than predicted. 2/10 -> 5.4/10**, no gate fired (fidelity 5, one clear of the cap). First ungated run since v25 — v26-v30, v33, v35, v36 all gated at 2. The judge: "the falsehoods are in the why, not in captures, threats or legality, which is the only reason" | kept |
| 80 | (v37) **The prompt-side half did not hold; the gate is what carried it.** Measured: on the 4 spoken turns where the reason was withheld, the model wrote one anyway on 3 of them. "Name the move and stop there" is a negative instruction and it went the way every other negative instruction has | Nothing — recording it. The prediction that instructions would not hold was made before the run and is confirmed | Do not spend another change on prompt wording alone. The two mechanisms that have moved this metric are withholding data and mechanical gates | kept |
| 81 | (v37) **A change I made earlier this session caused the remaining falsehoods.** The achievement-clause ladder retired a clause after two showings. The engine wanted a3 on plies 20, 38, 44, 46 for one reason (undefended bishop on b4); we stated it at 20, retired it, and the model filled the gap — "a3 addresses the isolated pawn on a2" (advancing it does not de-isolate it), twice "c5 directly addresses the isolated a2 pawn" (cannot touch it) | Ladder no longer withholds: clause always stated, framing escalates to "the same idea recurring". `ACHIEVEMENT_RETIRE_AFTER=2` -> `ACHIEVEMENT_REFRAME_AFTER=1`, no ceiling | **Not yet measured** (prompt-side, needs v38). The error was applying a lesson mechanism to a fact: a reason does not go stale, it is true again. A withheld fact produced invention, not silence — which is the same lesson as row 80 from the other direction | open |

| 82 | **Why the score plateaus even though we fix what the judge finds.** Asked by the product owner; answered from the rubric text and the four rubric-v2 runs, not from impression | Nothing yet — this is the finding that should redirect the next several changes | **Fidelity carries zero weight.** The rubric says it outright: "Fidelity gate-only (there is no credit for being true, only a penalty for being false)." The weighted sum is Diagnosis 25, Transfer Handle 25, Executability 15, Load Discipline 10, Stance 10, Stream Behaviour 10 — and recomputing v37 from those alone gives 5.37 against the judge's reported 5.4, so the reading is confirmed, not inferred. Nearly every change from v26 to v37 targeted fidelity. That work bought exactly one thing: it took the cap off, 2 -> 5.4, the largest single movement in the project. It is also **finished as a scoring lever** — fidelity 5 -> 10 is worth +0.00. Meanwhile the two heaviest dimensions, half the total weight, have never moved: Diagnosis 5/4/5/5 and Transfer Handle 5/5/5/5 across v33, v35, v36, v37 | kept |
| 83 | (row 82, continued) Why we ended up there, so it does not recur: the gate made fidelity the loudest finding in every review, and we answered the loudest finding every time. That was not wrong while the cap was on — no other work could score. It became wrong the moment the cap came off, which was v37 | Headroom, computed: Diagnosis 5 -> 8 is +0.79; Transfer Handle 5 -> 8 is +0.79; both is 6.95. Everything else combined is worth about +1.06 | Both are compositional rather than factual — Diagnosis wants the *thinking* failure named, not the board feature ("you moved a piece without checking it was defended", not "your bishop on c4 was undefended"); Transfer Handle wants a trigger attached to the principle, not the principle alone. That is the same shape of problem the composed-clause machinery already solves, so it is reachable without more prompt wording | open — TOP |

| 84 | (v39) **Diagnosis 5 for the fifth run running.** Judge: "on the blunders it names the right board feature ... but never the thinking failure ... It drops to anchor 4 on the small inaccuracies, where a full-length message goes to a secondary feature — king repositioning at 26, an isolated a2 pawn at 42 — while the game was actually being decided by pieces left en prise" | **Found a missing-data bug, not a wording problem.** Measured on the v39 transcript: of the 18 turns the coach spoke on, **zero** carried a Hanging Pieces or Threats section. Every one got piece placement, pawn structure and engine lines, and was asked to explain a blunder from that. It wrote about pawn structure because pawn structure was the only fact it had. Both sections now go into the move prompt, and where the engine says something of OURS is hanging it is named as the subject of the turn | **Not yet measured** (v40 in flight). Coverage measured BEFORE running, per row 87: fires on 13 of 18 v39 turns against 0 today. The data existed all along — the Coach already fetches this report for the guidance block, and both formatters already existed for the position prompt. Nobody had passed it in | open |
| 85 | (v39) Same finding, second half: the anchor for 8 is "names the specific process failure" | `diagnosis.missed_check` names the skipped question — moved onto a square they already covered / moved the only defender of another piece / ignored something already hanging / opened a line into the destination. Pure defender geometry, the same python-chess rules layer `verify.py` already uses to falsify the model's claims | **Not yet measured.** Coverage is low (3 of 18 on v39, gated on the engine supplying a refutation line), so it is a bonus rather than the lever. Recorded as such rather than counted twice | open |
| 86 | (v39) **Retraction of row 83's approach.** The judge on the piece-provenance line built in v39: "the one attempt at a cause, 'How this came about: your knight on g5 has not moved this game' (20), is **a piece-history fact, not a cause**" | Provenance removed from the prompt entirely, not demoted. Load Discipline penalises a second idea and a fact that is not the cause is exactly that. What survives from the record is the prior-warning note, which is about the student's pattern rather than a piece's travels | Correct, and worth stating plainly: **we followed a judge prescription and it did not pay off.** v38 told us to supply "which earlier move made this refutation possible"; we built exactly that; v39 called it not-a-cause. Reinforces the standing rule that prescriptions get checked against the record, not implemented on trust | kept |
| 87 | **Process fix, from the v39 post-mortem.** Two consecutive changes shipped with coverage too low to register: the history line hit 2 of 18 turns, the attack-geometry check 2 of 18. A dimension grade is one verdict over all coached turns, set from the typical turn — so a change touching 2 turns cannot move it, however good those 2 become | Before spending a nine-minute run, count how many of the ~18 coached turns the change can touch. Under half, do not run it yet | Adopted. Applied for the first time on row 84 (13 of 18, run) and row 85 (3 of 18, shipped but explicitly not counted on). Retrospective check: the only change that ever moved the score, v37's authorship rule (row 79), is also the only one that applied to all 18 turns | kept |

| 105 | **The cause field built and wired (v43).** `error_diagnosis.py`: five classes from diffing `board.attackers()` across the two resulting positions — left undefended, moved the only defender, missed a free capture, stopped defending, opened a line. Each carries the habit that would have caught it. Recorded twice in BACKLOG.md and never built; the review's Change B; the product owner's filtering diagnosis from the other side | Cause now comes from the attack-map diff, with the refutation-gated `missed_check` and the generic hanging phrase demoted behind it. Takeaway keyed on the diagnosis class, routed through `composed_lesson` so the existing repetition ladder covers it. Weak classes gated out of the takeaway by `STRONG_KINDS` | **v43 = 5.0. Diagnosis 5, Transfer Handle 5, Load Discipline 4.** Neither target moved. Composition worked — 8 of 18 turns diagnosed, against 3 of 18 for the bolted-on version — and the deterministic counters moved: takeaway keys spread 11 ways, most-repeated down 5 -> 4 turns. Three defects in my own first wiring were caught by the existing tests: the takeaway bypassed the ladder, the weak classes displaced a phase-specific endgame lesson, and the hanging fallback clobbered the specific cause | kept |
| 106 | (row 105) **The gap is voicing, not composition. Measured: 13 of 18 prompts carried the cause, 8 reached the output.** The five that dropped it — plies 40, 46, 52, 58, 60 — are exactly the five the reviewer scored at anchor 4 for "pure X was stronger with no account of what went wrong" | v44 tried displacement: on a diagnosis turn the achievement clause and the undefended line stand down, so exactly one thing owns the turn. The review's Change B in one line | **v44 = 4.0, and REVERTED.** Worse on five of seven dimensions (Diagnosis 4, Transfer Handle 3 — the lowest ever recorded, Executability 5, Stream 4) and worse on my own metric: voicing fell 8 -> 4. Mean words per turn 53 -> 33. Removing two fact sources did not make room for the cause, it cut the message by 38% and the coaching went thin. One turn genuinely improved (ply 34 voiced "your king was the only piece guarding that square") and it was not worth the rest | reverted |
| 107 | (rows 105-106) **What the two runs establish together.** The cause field is right and its coverage is real, but the constraint is the word budget, not competition between facts | Nothing yet | Adding a fact source without removing one crams the turn (v43, Load Discipline 4). Removing two starves it (v44, everything down). So the next lever is the WORD LIMIT on diagnosis turns, not the fact set — currently 40-120 by tier, set when a turn carried no cause. That is a one-constant change and it is measurable by the same voicing counter | open — TOP |

| 108 | **`--seed` was decorative. Every report card ever run played the same game** — which is row 104's finding traced to its cause, and the product owner had raised overindexing on one game before it was acted on. `play_game` stored the seed in its metadata and never used it; `move_fn` was a deterministic `player.play(fen, depth)`, so the seed could not reach a decision | `SEED_OPENINGS` in `scripts/eval_coach_review.py` maps the seed to a starting position: 7 standard, 11 Italian, 13 Queen's Gambit, 17 Sicilian, 23 French. Deliberately NOT seeded move sampling, which would change how the student plays and cost us rerun reproducibility | Verified both directions: different seeds now produce different games, and the same seed reproduces byte-identically. Every ledger number up to row 107 rests on seed 7 alone; that is now a choice rather than an accident | kept |
| 109 | **The fallback rate across five openings is 34%, not 6%: one coached turn in three ships template text.** Measured by `scripts/eval_check_breadth.py` over the five fixed games — 24 fallbacks of 70 coached turns, against 6 of 99 in the August run of the same harness. Worst on the report card's own game, 5 of 10 | Nothing yet — this row is the finding | **`opponent_reply` is 20 of the 24.** Verified from `output/check_breadth_v44/breadth.json`: 44 rejected attempts across 20 distinct turns, every one of them failing twice and so falling back; the other two gate kinds account for 4. The coach was inventing an opponent's move, the gate was correctly blocking it, and the whole turn was then replaced. So the third of turns going to template text is essentially one defect, not a broad fidelity problem | kept |
| 110 | (row 109) **The invitation was ours.** The reply instruction carried a worked example on every turn — 'if an "Opponent's reply" is shown, name that single reply ("after your move, the opponent plays X")'. Conditional in words, an invitation in practice: the model was handed a sentence pattern with no move to put in it | `_REPLY_SUPPLIED` / `_REPLY_WITHHELD`: the pattern is offered only when a refutation line is actually rendered, and where it is not the prompt says so plainly. Same shape as v37's reason clause (row 79), which is the only lever that has ever moved a score | **Coverage verified before spending the run, and it is high: 18 of the 20 broken turns had no refutation line at all**, so the fix reaches them; on the other 2 a reply WAS supplied and the model misnamed it anyway, which is what the gate stays for. Cross-referenced per (game, ply) between the breadth run and `scripts/eval_prompt_coverage.py`. **Prediction registered before measuring: fallbacks 24 -> between 6 and 20.** 6 if withholding holds as designed; nearer 20 if the negative half leaks the way row 80 measured it leaking. **Output not yet measured** — the LLM run is blocked on expired tunnel auth | open |
| 111 | (row 110) **A cheap coverage instrument, because the rule in row 87 had no tool behind it.** "Count how many of the ~18 turns a change can touch, before running" required a report card or a breadth sweep, both of which need the remote model, so it was applied by hand or not at all | `scripts/eval_prompt_coverage.py`: replays the same five games with the engine alone and reports which conditional prompt sections were present per coached turn. No model call, no judge, ~80 seconds for all five games | Validated by reproducing the breadth run's turn population exactly — 10/13/13/19/15 coached turns per game, matching `check_breadth_v44` — which also settles that the five games really do replay identically, as that harness claims. Two numbers arrived with it: the reply pattern is now withheld on **30 of 70** turns (43%), and the composed fallback would carry a cause on **45 of 70** (64%) across five openings, against 13 of 18 measured on the one game. Cause classes: missed capture 18, left undefended 13, stopped defending 10, moved the only defender 2, opened a line 2 | kept |

| 112 | (row 110) **The reply fix worked, and uncovered a bigger defect underneath it.** Fallbacks 24 of 70 -> **11 of 70**, inside the 6-20 predicted before the run, and `opponent_reply` collapsed 44 -> 4. But two kinds that fired ZERO times in v44 appeared: `illegal_move` 14, `move_claim` 10 | Nothing yet — this row is the finding | **All 24 were OUR errors, verified on the board.** Every one of the seven "illegal" moves is legal after the student's move (exd4, bxa6, Qxg5+, Nf3, dxc5, Rdxc8, exd4); both consequence claims checked are true (after Be2 the opponent really can play Rxg5; after Bd3 a piece on a4 really does attack a7). **Eight of the eleven fallbacks had no other cause**, so the gate was swapping correct coaching for template text. Reading them needed a harness change: it stored the violation window, not the sentence, and "Be2 … take your pawn on g5" is identical whether the coach claimed Be2 takes it or correctly said the opponent could | kept |
| 113 | (row 112) **Whose move is this was being decided by grammar, and the grammar list could not keep up.** The cue required the colour word and the verb to be ADJACENT, so every form the coach actually writes walked past: "lets Black capture your bishop on d4 with exd4", "which Black immediately exploits with bxa6", "allowing Black to recapture with Rdxc8", "allowed White to defend your pawn on h2 with Nf3", "letting them capture your pawn on g5 with Qxg5+" | Ask the board, not the sentence. After the student's move it is the OPPONENT to play, so a token that parses there and not in the position as given can only be their reply. **No new cue words** — rows 27 and 95 are two earlier rounds of widening the wording, each of which produced false positives of its own. Four narrower defects fixed with it: a consequence whose subject is the opponent charged to our move; the attack-claim window crossing a full stop; a RECOMMENDED move read as their reply where it recurred beside the word "opponent"; and "the opponent's STRONGEST reply" claiming their move for us | **Fallbacks 2 of 70 (3%), against 6% in August and 34% two runs ago. Zero leaks. The only two firings left are the two real coach errors** — a white pawn called "your pawn" while the student is black, and a rook said to threaten its own side's pawn. Replaying the 13 rejected turns: 11 clean, 2 still flagged and both genuine. Worth stating plainly: this is a ROUTING fix, not an exemption. The move now goes to the opponent-reply checker, which tests legality, the check marker, what it attacks and what it captures against the right board — it gets MORE checking. Both halves share one predicate and one argument list so no token can be dropped by both | kept |
| 114 | (row 113) **The leak counter was not running the gate's checker.** It called `check_text_fidelity` without `played_uci` while the gate passed it, so on the first v46 run it reported **6 leaks on shipped text that was clean** — the missing argument made it flag opponent replies the gate had routed correctly | Passes `played_uci` and keeps the offending sentence, so a leak can be read rather than re-run for | Confirmed the mechanism before re-running rather than after: the same sentence is clean with the argument and `illegal_move` without it. Re-run gives leaked=0. The general lesson is the one `generate_verified` exists for — every hand-written mirror of the shipping path drifts, and this one had drifted quietly since it was written. It only became visible because this change made the two configurations diverge more | kept |

| 115 | (row 113) **Asked by the product owner about the 2 turns still falling back: "are we not trying to fix this?" Both were OUR sentence, not the model's.** Read straight off the prompt: "the opponent's strongest reply is Nf3, **defending your pawn on h2**" with the student playing Black and h2 holding a White pawn; and "Rf7, **moving your rook to f7** on the half-open f-file" for a black rook. The model voiced both faithfully | The clause builder carries TWO possessives and only one was parameterised. `target_possessive` covers the pieces a move acts against — correct, and correctly flipped to "your" for a reply that captures the student's piece. The moving side's OWN pieces were hardcoded "your", which is right for the student and wrong for every clause describing the opponent's reply. Eight branches affected: escape, defend, castle, open file, mobility, king activity, extra defender, development, plus the centre-control counts, where it is a swap rather than a substitution | **Coverage measured before fixing, and it was six times what the gate showed: 11 of 70 coached prompts (16%) carried a clause of this kind**, not 2 — the ownership check only fires on some phrasings, so 9 went unremarked. Fallbacks 2 -> **1 of 70**. Clause snapshot byte-identical across 5062 positions, which confirms the blast radius: the student-facing wording is untouched and only the opponent-reply path changed | kept |
| 117 | (rows 110-116) **The arc, one number per run, same detector and same five games. Fallbacks 24 -> 11 -> 2 -> 1 -> 0 of 70; gate firings 54 -> 32 -> 4 -> 2 -> 0.** v48 is the first run in the project's history where the fidelity gate does not fire once across five games | Nothing — this is the readout | **Every one of the four causes was authored here, not by the model.** A reply pattern offered with no reply to put in it; a checker deciding whose move a token was from grammar; a possessive naming the opponent's piece as the student's; an instruction pointing at a threats section this prompt has never contained. None was a model failure and none was a Blunder failure. The honest limit on the claim: this measures FIDELITY, and fidelity carries zero weight in the rubric (row 82) — so expect no score movement. What it buys is that the coach now keeps its own sentences, on 24 turns in 70 that previously shipped template text | kept |
| 116 | (row 115) **The last fallback, and the third instance of one pattern.** The reply instruction ended "and what it wins, **using the threats shown**" — and this prompt has never contained a threats section: `_format_threats` is wired into the position prompt and the Socratic prompt only. So on every serious-tier turn the model was told to describe the consequence from data that was not there, and it obliged: "Rf7, threatening their pawn on g6", where g6 is a black pawn lifted from the placement block and a rook on f7 does not attack it | Replaced with the authorship rule from v37 (row 79): the composed line beside the reply is the ONLY description of it permitted, no threat, capture, piece or square that line does not name. Correct whether or not that line carries a clause | **The pattern, stated plainly: ask for a claim we have not supplied and the model supplies one.** Three times in this sweep — the reply template with no refutation (row 110), the possessive naming a piece we had mislabelled (row 115), and now a threats reference pointing at nothing. Every one was authored here, and each was invisible until the run was read sentence by sentence rather than counted | kept |

| 118 | **First blind A/B that is not a wash. v43 vs v48: 11-6 to v48, 1 tie, p=0.332.** Same game, same 18 spoken plies, position-for-position identical, 15 of 18 pairs unanimous. Against 8-7 (v33 v42) and 6-5 both ways (v39 v42), all p=1.000 | Nothing — this is the readout | **The pre-registered prediction held: v48 takes 4 of the 5 turns that changed in kind** (plies 14, 46, 52, 60 won; 40 lost). Those five went from a two-sentence template — "a3 was stronger here — attacking their undefended bishop on b4. Worth remembering: …", 17-32 words — to 53-87 words of prose naming what went wrong. So the fidelity work IS visible to a reader once it changes a THIRD of the turns rather than two of them, which retro-fits row 101 exactly: the earlier nulls were "not large", not "no effect". Also fixed a reporting bug found here: the unanimity counter tested `len(set(votes.values())) > 1`, true of any tally containing a zero, so it called a clean 3-0 a split and reported "18/18 not unanimous" when 15 of 18 were unanimous | kept |
| 119 | (row 118) **The six turns v48 LOST, and the retraction they force. Withholding the opponent's reply removed claims that were TRUE.** On plies 28 and 34 no refutation line was supplied in either run; v43's model invented one anyway — "the opponent plays Rxg5, winning your undefended pawn on g5", "Nxc4, winning your bishop" — and both passed the gate, so both were true and checkable. v48 obeys the withhold and says nothing concrete. The judge, unanimous on both: "it names the concrete refutation (Rxg5)" | Nothing yet | **This qualifies rows 110 and 112.** The withhold was measured on the fallback counter, which can only ever see the invention that was FALSE; the same instruction also suppressed invention that was true, and the counter is blind to that by construction. Not a reason to revert — a reason to close the data gap instead. The engine is not asked for a reply on these turns; asking it for the best move in the position AFTER the student's move is the engine answering "which move is best", which is its job, and then nothing has to be invented. Two further losses (plies 20, 30) are from my own v48 rewording of `_REPLY_SUPPLIED`: "omits the cost" at 20, and at 30 a duplicated "The opponent's reply after your move is exd4" the judge marked down explicitly | open — TOP |
| 120 | (row 118) **The judge called two of v48's CAUSE sentences false and it was right both times — verified on the board — and our gate does not check this class at all.** Ply 34 "you left your pawn on g5 undefended, which was the only piece guarding it": g5 had zero defenders BEFORE the move as well, so the move caused nothing, and the real cause was Ke2 vacating d1, the only defender of c2. Ply 40 "you allowed your pawn on c2 to become undefended": c2 was already undefended | Nothing yet | **Both trace to our prompt, not to the model's imagination, and the v43 prompt was byte-identical on ply 34 — its model simply happened to keep the two apart.** Two defects. (a) The section header reads "--- Undefended AFTER your move ---" over what is a STATE, so the model reads causation into it; we already diff the attacker sets, so we can say which it is instead of leaving it to be inferred. (b) The prompt carries TWO competing subjects on that turn — "Undefended AFTER your move: g5" and "How this came about: your king was the only piece guarding c2" — while the instruction says "the undefended piece named above is the subject of this turn". Different squares, no step choosing between them. That is row 103's filtering diagnosis with a verified instance attached: nothing SELECTS, so the student reads whichever the model happened to merge | open — TOP |

| 121 | (row 119) **Supplying the opponent's reply is fidelity-neutral, and the run that said otherwise was measuring our own checker.** `CoachingEngine.with_refutation` fills `refutation_line` from a search of the position after the student's move — the engine answering which move is best, so no chess logic moves here — and it is free in the Coach, which already fetches that report. Reply supplied on the report card went 3 of 18 turns to 17 of 18 | Nothing to undo. Three checks fixed instead: placement now judges a claim against the position as given OR after the student's move (third check to need that today); the defence check will not pair a claim with an earlier move when a recommendation sits between them; the reply's capture-victim window stops at a full stop | **Five games, same detector: fallbacks 0 (v48) -> 9 (v50) -> 1 (v51), firings 0 -> 19 -> 2.** Eight of the nine v50 fallbacks were OUR false positives, verified on the board — "bxa6, capturing your queen on a6" after Qxa6 took the knight standing there, "Rdxc8, capturing your rook on c8" after Rxc8. The one that survives is a real coach error the gate should catch: "Ka3, attacking your undefended bishop on d2", where d2 holds the OPPONENT's bishop and a king on a3 cannot reach d2. Naming the reply did not make the coach worse — it made the coach describe the position its move PRODUCED, which three checks could not read | kept |
| 122 | (row 121) **Process, and it changed the decision rather than the speed.** On the single-game report card there were two such turns and I had begun reverting the reply supply on them. The five-game sweep produced nine, and nine made the pattern unmistakable | The revert was abandoned; the checks were fixed | The standing rule against concluding from one game (working agreements, raised by the product owner before it was written down) saved a WRONG decision here, not a slow one. Also voids the v48-vs-v50 pairwise: that comparison measured a build in which two report-card turns shipped template text because of the checker bugs since fixed, so 11-6 to v48 describes nothing that now exists | kept |

| 123 | (row 121) **Fidelity-neutral is not the same as better, and on quality the reply supply LOST. v48 vs v51: 10-6 to v48, p=0.455, 5 of 18 pairs split.** Same direction as v48 vs v50 (11-6), so fixing the checker did not change the verdict | Nothing reverted. The reason is legible in the judge's prose and it is a design error of mine, not noise | **`refutation_line` means "the move that makes the student's move bad" and I filled it with "the engine's best move here", which on an ordinary turn is just a good move.** Then the prompt led with it. Five turns marked down for exactly that: "The opponent's answer: they play Rg8, moving their rook off h8 where it was attacked" opening a turn about a missed capture ("narrates an irrelevant reply"); "they play b6, preparing to develop their pieces" ("filler narration"); ply 56 and 58 likewise, both burying a double-attack lesson under a quiet reply. Leading with a non-event buries the lesson — and "lead with it" was my own item-4 wording | kept |
| 124 | (row 123) **The reply now has to PUNISH: a capture or a check, nothing else.** Rules geometry, so it stays ours; whether the move is best remains the engine's answer | `_reply_punishes` in `engine.py`, gating `with_refutation` | Coverage measured at each step rather than after. Unconditional: 70 of 70 turns carried a reply. First filter, "capture, check, or attacks something of ours that nothing defends": **59 of 70**, and it let the narration straight back through — in a middlegame almost any move points at some loose piece, so it admitted 19 of the 30 turns that had none. Capture-or-check only: **52 of 70**, which is the original 40 plus 12. Every reply the judge rewarded naming was a capture; every one it marked down was a quiet move. A narrow test that is right beats a wide one that is usually right, because this field is what the coach LEADS with. **Quality not yet re-measured** — and it should be measured on more than the one game that produced this diagnosis | open — TOP |

| 125 | (rows 123-124) **Five games, both arms, ~90 pairs: naming a PUNISHING reply wins 28-20 (58%), p=0.312, and 5 of 5 seeds lean the same way.** Per seed — standard 6-5, Italian 3-2, Queen's Gambit 8-6, Sicilian 5-2, French 6-5. Arm A is the v48 build run from a git worktree at `8e5e36f`, verified to import its own `src` so it is the old code and not the new code with an old label | Keep the reply supply, gated to captures and checks | **Directionally consistent, not proven, and the earlier verdict is retracted.** On seed 7 the UNGATED version lost 6-10 and the gated one wins 6-5 on the same game, so "v48 is better" (row 123) was measuring my design error rather than the idea. Two readings of the aggregate, and both belong in the record: the pair-level sign test does not clear 0.05, while the SEED-level direction being unanimous is p≈0.06 on its own — weaker evidence than a p-value but not nothing, since one game's 6-5 is easy to get by luck and five games all leaning is not. Also 33 of 81 pairs are TIES (41%), which is the gating working: the two builds now differ only on the ~12 turns in 70 where a real punishment exists | kept |
| 126 | (row 125) **The sensitivity cost of this instrument, computed rather than guessed.** At a 58% win rate, p&lt;0.05 needs about 200 pairs — roughly twelve games per arm, four hours of runs at today's rates. 48 decisive pairs gives p=0.312; 120 would give 0.082 | Nothing — a planning number | So the honest limit: **this harness can prove a large effect and can only ever suggest a moderate one.** Row 99 estimated one game detects "large, consistent" only and row 101 said 3-5 games would be needed for moderate; five games turns out to be enough for a direction and not for a proof. Worth knowing before the next change is judged by it — and worth preferring changes that touch most turns, since those are the only ones this instrument can settle | kept |

| 127 | **One subject per turn — the first verified fix to the filtering problem (row 103), and it was a contradiction WE wrote.** The prompt carried "Undefended AFTER your move: your pawn on g5" and "How this came about: your king was the only piece guarding c2", then asserted the loose piece was "the subject of this turn". Different squares, nothing choosing. Ply 34 came out false, TRUE, false across three runs on a byte-identical prompt | The cause wins when the two disagree: the loose piece is relabelled "Background (NOT the cause)" and a new `_FOCUS_CAUSE` instruction names the cause as the subject. The old same-piece test compared whether the phrase appeared verbatim inside the cause sentence, which fails exactly when the squares differ — it now compares squares. Header also says whether the move CAUSED it, computed from the defender counts, instead of implying it with "AFTER" | **Five games, deterministic counter, no judge: turns opening on the wrong piece 4 of 20 -> 1 of 14; turns not opening on the cause at all 9 of 20 -> 3 of 14.** Ply 34 correct for the first time: "you left the pawn on c2 undefended, allowing your opponent to capture it with Nxc2". Kept the background fact rather than deleting it — v44 deleted fact sources on diagnosis turns and words fell 53 to 33 with every dimension down, so the lesson was to stop two facts competing for the subject, not to say less | kept |
| 128 | (row 127) **Checked on a SECOND model, because the product owner asked whether prompt changes are model-immune — and multi-model checking is something this project used to do.** `docs/llm-probe-findings.md` compared five models in April; `scripts/eval_run.py` takes `--models` as a list. The report card takes `--model`, singular and required, and it became the only instrument we ran | Nothing yet — the report card should take `--models` | **With the fix in, 0 of 18 turns open on the background piece across two models** (qwen3:14b 9 of 9 on the cause; gemma4:12b-it-qat 6 of 9 on the cause, 3 on neither), against 4 of 20 under qwen without it. The honest gap: no gemma run from BEFORE the fix, so gemma's zero cannot be attributed to the fix. Gemma differs in style — leads with the habit question on 2 of 9 turns, emits markdown bold, and produced a mind-reading opener, which is row 91 appearing under a second model. **Same failure shape as `--seed`:** a capability existed, the newer harness fixed that dimension to a constant, and nobody noticed because the newer harness was the only one in use | open — TOP |

| 129 | **The mind-reading regression (row 91) fixed at source: WE asked for it.** The inaccuracy tier's instruction opened with "acknowledge the intent in a few words". The prompt states what a move DOES and never why it was played, so there was no intent to acknowledge and the coach invented one — "your move aimed to develop the bishop", "you were looking to activate your rook" | The instruction now says to name the stronger move and what it does, and says plainly not to guess what the student was trying, aiming or hoping to do | **Six real cases to zero, across three runs and two models** — gemma seed 7 3->0, gemma seed 23 2->0, qwen seed 7 1->0 — with nothing else moving: clean% stays 100, words 82->81, 85->85, 66->66. Fourth instance of one pattern in this session: ask for a claim we have not supplied and the model supplies it. The other three were the reply template with no reply, a threats reference pointing at a section this prompt has never had, and "what it wins" on a reply that wins nothing | kept |
| 130 | (row 129) **Two corrections to the record, and the second one is the useful lesson. Row 91's "1-2 per game" was wrong, and the code already knew.** `_check_intent_attribution`'s own docstring says "measured across four runs it invents 5-7 of these per game, and most are unfalsifiable" — so the ledger and the source disagreed for months and the ledger was the one being read | Counter rebuilt as a pattern over the SHAPE — an intent verb applied to the student — instead of a list of phrases, plus a guard for the hypothetical form | **The counter was measuring its own examples.** It held eight literal strings, all variants of "I see you're trying" (26 hits across the stored transcripts), and missed the dominant forms entirely: "your move aimed to" 55, "you were looking to" 26, "you were aiming to" 23, "you were trying to" 11. **A detector built from the examples someone quoted will measure those examples and nothing else.** Second correction, against my own first count: 148 instances sounded alarming and was dominated by OLD transcripts — today's build sits near 1 per game on qwen, so the change is worth making and could never have been proven on the model we ship. Said before the runs, not after (row 87). Third: my widened pattern then over-matched the takeaway hook — "NEXT TIME you are looking for a way to improve" is advice, not a claim — which is why the counter now checks what precedes each match | kept |

| 131 | **The endgame chain was broken at BOTH ends, and nine reviews across two models named it the single highest-leverage change.** Blunder's theme classifier had six labels and every one was an opening/middlegame concept, so a rook ending, a passed-pawn ending and opposite-coloured bishops all came back "general play" — which biases nothing — and a DRAWN KPK came back "king attack" because a check existed. Meanwhile `king_safety.description` was generated unconditionally and read "missing d,f-pawn shield, open file near king" about a correctly centralised king, on 60% of prompts | Engine (Blunder `0d62a89`, delegated to a sub-agent and verified with our own audit script): an endgame theme vocabulary, the middlegame labels gated out of the endgame, and the king-safety prose/flags phase-gated on the SAME notion the score already used — `phase()` exposed as a free function so the words cannot disagree with `eval_breakdown`. Coach: all 12 engine themes mapped, plus two bank entries with discriminating features (rook behind the passer keyed on `passed_pawn`, converting a lead keyed on `material_lead`) and the king entry enriched with cutting the king off | **Verified end to end, engine only, no judge: endgame positions now select 2-3 endgame entries where they previously selected one or none, and the opening/middlegame controls are byte-identical.** Scores untouched — every `info depth` line and `bestmove` identical on all six audited positions. The bank went 3 endgame entries of 22 to 4 of 24, which is the remaining gap: endgames are the LARGEST phase in the report card (13-18 turns of 40) and still the thinnest in the bank | kept |
| 132 | (row 131) **Two process notes, one of them a correction to my own rule.** I applied "count coverage before building" to our code all session and then did not apply it to the dependency: I audited the engine, delegated the work, and only afterwards counted the bank's endgame entries — the number that decides what the engine change is worth | Nothing — recording it | Owner's ruling, which overrides the lesson I drew: adding a field to the engine's coaching output ahead of consuming it is fine when we own both repos, so do not gate engine asks on coach-side readiness. Second note, and this one holds: **two entries keyed on the same feature set are indistinguishable to the selector, which recreates the monoculture one level down.** That is why this added two entries with discriminating features and ENRICHED the existing king entry rather than adding rivals to it. The residual is visible and recorded rather than hidden: "rook cuts the king off" and "king activity" both map to `phase:endgame`, so neither can steer to a specific rook-technique entry | kept |

| 133 | (row 131) **Why the endgame work reached no student, found by measuring instead of guessing: across 28 endgame turns the coach was HANDED an endgame lesson on one.** Not a voicing failure and not the bank. `composed_lesson` keys on the move's effect category, `capture` wins on 14 of 24 endgame turns, so `capture:phase:endgame` is taught once, reframed on the second showing and retired after — and every later endgame turn closed on nothing | First attempt: when a lesson is RETIRED, substitute the position-matched guidance principle instead of going silent | **14% -> 17%, one turn in 28. Two reasons, both now located.** The substitution fires only at RETIREMENT, and endgame turns sit overwhelmingly at the REFRAME step — where the instruction re-serves the same lesson as "the SAME idea as earlier". That reframe IS the repetition the reviews describe. And when it did fire it took the top-ranked guidance entry, which on 5 of 9 turns was a middlegame principle ("pin", "capture value") — a fix for the middlegame monoculture serving more middlegame. Both were predicted before the run and both held | kept |
| 134 | (row 133) **Second attempt: substitute at the REFRAME step, and prefer a phase-tagged entry.** Naming a recurrence is kept as the fallback for when nothing better exists — the change is that having something better now wins | `_build_takeaway_instruction` substitutes at `times_taught > 0` rather than `>= LESSON_RETIRE_AFTER`, and `_guidance_takeaway` prefers an entry carrying the current phase over the top-ranked one | **It moved, and it is the first thing this session to move the student-visible endgame number. Over the same 14 endgame turns in two games: 2 (14%) -> 3 (21%) -> 5 (35%) across the three attempts**, and turns GIVEN an endgame lesson 2 -> 6. Clears the bar of 4 of 14 registered before the run. Seed 7 did the work (1 -> 3 voiced, 0 -> 3 given); seed 13 held at 2 voiced while gaining a given lesson, so one turn was handed an endgame lesson and did not say it — the voicing gap seen at v43, again. Limits: 35% is not most turns, nine of 14 still close on the mantra or nothing, and this is two games on one model. What made the difference was measuring the FAILED attempt rather than reasoning about it: the substitution had to happen where the turns actually sit, and it had to prefer a phase-tagged entry or it swapped one middlegame lesson for another | kept |
| 135 | (rows 133-134) **Seven detector errors in one session, each caught before its number was believed, and one of them nearly changed a conclusion.** The endgame counter took a fraction of the characters as the "takeaway" and sliced the best endgame turn in the transcript in half; it did not know the composed table's own `(capture, endgame)` wording; it matched only the original takeaway instruction and reported every substituted turn as having been given nothing; and it looked for "Same idea as earlier" where the prompt says "SAME idea as earlier", which classified reframed turns as having no lesson at all. A guidance-block check used marker strings that appear in no prompt and concluded guidance was missing on every turn when it is present on all 34 | Nothing — recording the pattern | **A detector is not evidence until it has been checked against a case whose answer is known independently.** Every one of these was found by reading the turns the counter flagged rather than trusting the count, which is the only reason the numbers in rows 131-134 can be relied on. The one that mattered: "guidance is missing in the shipping path" was briefly a third root cause and was pure instrument error | kept |

| 141 | (the owner's ask: "see how we progress/regress over time, over multiple games, multiple LLMs") **The tracker is built, and its first real use found a regression in a change we had already called a win.** 20 runs on ONE identical build — 4 models x 5 openings — gave the noise floor, which was the point before any chart: **the opening alone moves `cause_given_pct` by 27 points, `lesson_conc_pct` by 19 and `spoke` by 12 turns, while the MODEL moves those three by exactly zero.** Those three are computed from our own prompt, so they are coach-side by construction. `words_per_turn` is the opposite: 60 to 181, dominated by the model (hermes3:8b writes three times what qwen3:14b does). `cause_voiced_pct` swings 50 points either way, which finally explains the jagged line I had flagged and could not account for — it was never a coaching signal. Also: qwen3:8b scores clean 100% and voicing 94-100% despite the model scorecard rejecting it for inventing boards, which reads as the composed-clause architecture removing its opportunity to invent rather than the metrics missing it | `scripts/metrics_history.py` emits `docs/coach-metrics-history.tsv`, a legend, and a self-contained HTML dashboard. `--check` catches staleness; `--guard` fails on a regression. 14 tests | **`lesson_conc_pct` went 14% -> 25% between v52 and HEAD at the same seed, and the cause is row 136's anchoring fix, which I reported as a clean win.** The breakdown: it filled the 2 previously-silent turns and raised distinct lessons from 9 to 12, both good — but it substitutes "isolated pawn" FOUR times where nothing previously repeated more than twice. The teach/reframe/retire ladder tracks the *composed* lesson; the *substituted* lesson has no tracking at all, so an anchored guidance entry can be reused without limit. Unfixed | measuring |

| 142 | (row 141) **Four ways the dashboard flattered the numbers, each caught by constructing the case rather than by reading the output.** (1) The trend chart plotted all 68 rows on a version axis, and matrix runs are named `gemma12_seed11`, so the version parser returned 12 for them — twenty spurious points mid-axis under a heading reading "v2 to v52 (68 runs)". Nothing raised; the picture was just wrong, and the product owner spotted it, not any of my checks. (2) Panels charted raw COUNTS against a hardcoded "of 44 turns" axis, true only of the standard-start game — the five openings run 23 to 51 plies, so `spoke=8` and `spoke=18` are 35% and 41%, close, where the counts look far apart. (3) Several runs of one version drew several points at the same x, so the line doubled back vertically instead of averaging, and the headline card reported whichever row sorted last as "now". (4) Cards compared the latest value against the worst ever, which reads "flat throughout" when the latest value IS the worst — a simulated `clean_pct` fall of 100 -> 97 came out as unchanged | `render_svg` now RAISES on non-history rows rather than filtering silently; every panel is a ratio with the denominator in its column name; `_agg` gives one point per version with a min/max band; cards are a state machine (best / WORST / middling / unchanged) | **Every one of the four failed in the flattering direction.** A wrong chart is worse than no chart, which is why the guard raises instead of quietly dropping rows. And the lesson about my own checking: I verified the file was generated, the row counts, the tag counts, ruff and pytest — and never that the picture made sense. The sanity check I wrote for the headline cards was the right instinct and I did not extend it to the chart | kept |

| 143 | (row 141) **The regression the tracker found, closed.** Row 136's anchoring fix raised `lesson_conc_pct` from 14% to 25% because the teach/reframe/retire ladder tracked only the COMPOSED lesson — a substituted guidance entry could be re-served without limit, and "isolated pawn" closed four turns where nothing had previously repeated more than twice | `prompts.substituted_lesson_key()` decides which lesson a turn will close on, in ONE place, and `coach.py` increments the same counter for it. Substituted lessons now obey `LESSON_RETIRE_AFTER=2` like composed ones. The single decision point is deliberate: coach.py reimplementing the choice is the "two copies of one check handed different arguments" shape that caused four earlier defects | **`lesson_conc_pct` 14% (v52) -> 25% (HEAD) -> 12% (v53), distinct lessons 9 -> 12 -> 13, and it holds across all five openings at 10-12% rather than on the one seed that produced the diagnosis.** `metrics_history.py --guard` passes. So the fix is below the pre-regression level and the variety gain from row 136 is retained | kept |

| 144 | (row 136) **The confinement falsehood now has a check, and building it found a second hole in my own first version.** Row 136 recorded "Re4, which cuts the king off" at ply 58 with the black king holding the same five squares before and after, and nothing verified it because no check covered confinement | `_confinement_violations` measures one thing — how many squares the enemy king can legally move to, before against after — and one regex covers the whole family: cutting off, trapping, confining, boxing in, taking squares or mobility away. Added to `GATING_VIOLATION_KINDS` | **The hole: the side-of-the-king alternation listed enemy / opposing / their / black's / white's and NOT "opponent's" — while the prompt instructs the model to "refer to Black as their opponent", so that is the wording it reaches for first.** Found in real output, not in a constructed case: v53 seed 17 ply 65, "prepare to cut off the opponent's king", where the black king on g8 has the same three squares (f7, h7, h8) before and after `Kc2`. Now one shared `_THEIR_KING` pattern used by all four phrasings, both apostrophe forms, so the next phrasing does not need a fifth edit. A genuine cut-off still passes (Ra4 taking a king from 8 squares to 5). 4 tests. **Measured over the 73 stored transcripts: adding "opponent's" catches three more false claims, all on seed 17, and `clean_pct` FALLS — 100 -> 95 on hermes8_seed17, 95 -> 89 on qwen14_seed17 and on v53_seed17.** Worth stating plainly because it is the opposite of how the number usually moves: the coach did not get worse, the instrument stopped missing. Any comparison across this commit is measuring two different checkers | kept |

| 145 | (our finding, while investigating row 146) **Corrupt bytes were reaching the model on 72 of 81 prompts, and had been since at least v13.** Every prompt carries the engine's own description of its move as the ONLY reason the model is permitted to give, and that line read `king safety â€” repositioning the king`. We start the engine with `text=True` and no encoding, so Python decodes its UTF-8 output with the machine's locale — cp1252 on this box — and every em dash the engine sent arrived mangled | One `_spawn()` in `engine.py` used by both protocol classes, passing `encoding="utf-8", errors="replace"`. One function rather than two call sites, so the decoding cannot be right in one and wrong in the other | **Proven by running, not reasoning: the pre-fix and post-fix spawn were run side by side against the same child process, pre-fix mangled and post-fix clean, and both new tests fail when the fix is reverted on the live file.** The test asserts the CONFIGURATION (`encoding`, and `errors == "replace"`) rather than only a round-trip, because a round-trip passes before the fix on any UTF-8 runner — which is the whole of CI, so it would have protected nothing there. Quality effect not measured: no run yet exists with clean labels | kept |

| 146 | (row 140's `open` column) **Five of 81 spoken turns let the MODEL pick the lesson topic. Measured, then deliberately not fixed** | Nothing. Coverage counted before building, per row 87 | **Two of the five are king moves the detector declines CORRECTLY** — `Kc2` goes b2->c2, which is no nearer d4/d5/e4/e5, and `Ke2` is a middlegame move that the endgame gate blocks on purpose because an early king step is usually forced. **Three are pawn pushes and only one of them is a passed pawn**, so a passed-pawn-advance detector would reach 1 of 81 spoken turns and 1 of 32 endgame turns — and that turn's output is already right without it ("push your passed pawn on the a-file… can I support it with my king and advance it?"). What the investigation did find is worth more than the thing it was looking for: **on 11 of 81 turns nothing about the recommended move is verifiable, so the prompt hands over the engine's bare CATEGORY LABEL as the only permitted reason and the model restates it** — "c5, which improves your pawn structure", "Ke2, which improves king safety by repositioning the king". That is exactly the label problem `_quiet_move_effect` was written to remove, still live on 14% of turns. To BACKLOG | open |

| 147 | (the commit-checklist rule that a check reading gitignored artefacts protects nothing) **The breadth sweep was the fourth instance of one check handed different arguments than its twin.** `eval_check_breadth.py` ran the fidelity checks without `position_after` or `after_report`, so every check that needs the resulting position was silently inert in the sweep while the report card ran it fully | The sweep passes both. `.gitignore` un-ignores exactly 73 transcripts — `coach_review_v*` and `matrix/*`, transcripts only — so the checks that read them read committed inputs on a clean checkout | 11.0 MB raw, about 1 MB compressed. The `prompt` field is 83% of each file and is deliberately kept: it holds the composed cause and the closing instruction, which is what `eval_hard_metrics.py` measures. **Not yet re-measured with the sweep fixed**, so no claim about what it will now catch | open |

| 148 | ~~(rows 143-147) v54: 60% of the coach's prose is not reproducible~~ **RETRACTED IN FULL — see row 152. Do not cite this row.** The v54 sweep was real and its prompt-side result stands: every counter computed from our own prompt is identical to v53 on all five seeds (`spoke`, `plies`, `cause_given_pct`, `lesson_conc_pct` 10-12%, `lesson_open_pct`). Everything this row concluded about reproducibility, about the encoding fix having no effect, and about `--guard` being unsound was wrong, and wrong for one reason: **v53 and v54 are not a repeat.** 72 of the 81 prompts differ, because the comparison spans the encoding fix. The text differences measured the code change, not the model | Nothing. A bad measurement | The correct figures are in row 152, from twelve actual repeat runs | **retracted** |

| 149 | (the owner's ask, after this session proposed a guard fix that changes zero coached turns ahead of a defect affecting forty) **The backlog recorded WHAT to do and never what doing it would buy, so the cheapest item won every time** | Every actionable heading now carries **Category / Gives / Priority**, `scripts/backlog_index.py` generates the summary table from those fields, and `tests/test_backlog_index.py` fails when an item lacks one or the table drifts. Priority is defined as impact and P1 is required to state a number | **The review it forced found three items marked OPEN that had been fixed the same day** (the breadth sweep, the confinement check, the substituted-lesson cap) and produced 10 open actions: 3 P1, 6 P2, 1 P3. The ordering it produces contradicts what I proposed an hour earlier — coaching the good moves is P1 at 40+ of 81 turns, the lesson mismatch P2 at 9, and the guard tolerance P3 at zero, which is where I had it first. Generated from the fields rather than maintained beside them, because two copies of one thing drifting apart has caused four defects here. Verified by deleting a `Priority` line from a COPY: the checker exits 1 naming the line | kept |

| 152 | **Retraction of row 148, and the only honest reproducibility measurement in this ledger.** Twelve runs — six repeats each of seed 7 and seed 17, one build, one model, `--no-judge` | Nothing coach-side. A measurement, and the tooling in row 153 | **Within a cell, every counter has a standard deviation of ZERO.** `clean_pct`, `cause_given_pct`, `cause_voiced_pct`, `lesson_conc_pct`, `lesson_open_pct`, `spoke`, `plies` — identical across all six runs in both cells. Only `words_per_turn` moves, by 0.3 words. **So the run budget is one run per arm for every metric on the dashboard**, which is the opposite of what row 148 implied. The real non-determinism is narrow and structured, not noisy: in both cells the six runs form exactly TWO distinct transcripts — run 1 alone, and runs 2-6 byte-identical to each other — differing on 7 of 18 turns (seed 7) and 5 of 19 (seed 17). Run 1 also reproduces v54 exactly. A batch-boundary effect, cause unknown, and it moved no counter. Three claims from row 148 die with it: the 60% figure (only 9 of 81 turns were comparable and 8 of those 9 were identical); "the encoding fix changed nothing" (seed 7 `cause_voiced_pct` is 100 in v53 and **92.3 in v54 and in all six repeats**, sd 0 — a stable 8-point drop, direction inconsistent across seeds since seed 23 moved +5); and "`--guard` is crying wolf" (the guard flagged exactly that drop and I dismissed it) | kept |

| 154 | **Every judge run this project has made was served by the wrong model, and the ledger's model column is fiction.** `--model` is a NO-OP in kiro-cli 2.22.1: a nonsense name produces the identical "[warn] failed to set model: Method not found", omitting the flag produces no warning at all, and `--list-models --format json` reports `default_model: auto` | Pinned via `.kiro/agents/judge-opus5.json` and `--agent`, chosen because it FAILS rather than falls back — an agent naming an unavailable model exits 1 with "Error: Internal error", verified both ways. `resolve_judge_agent` refuses to run when the agent file and the recorded label disagree. 5 tests | **This retroactively weakens the sonnet-versus-opus finding the "do not drop to a cheaper judge" rule rests on** (row 505's relabel work included): both arms may have been `auto`. The transcripts record the coach's model and never the judge's, so no stored artefact can settle which model wrote which review. Not recoverable; recorded so no future row cites a judge model for a run made before this commit | kept |

| 155 | (row 154, and the first review run with the internals corrected) **The architecture summary handed to the design reviewer described a system we do not have.** Its worst line: "these counts are DIAGNOSTIC only today — nothing is regenerated or blocked when violations are found", when `generate_verified` has for months retried once and then shipped deterministic text instead. It also predated the error-cause field, the takeaway ladder, the substitution path and `engine_trust`, and put the bank at ~20 entries against 24 | Rewritten from the code, plus a list of KNOWN OPEN DEFECTS so they are not re-proposed as discoveries, and the owner's anchoring constraint written into `CONSTRAINTS`: any measure must be computable from the board, our composed text, move tokens, squares, eval drop, phase or earlier turns — never from matching phrases in the model's output. `_ARCH_TASK` now REQUIRES a measure for every defect, with its current value, a fixed threshold, and what the measure would miss | **The corrected review found three real defects in one run, against nine output-only reviews that found none of them** — and it obeyed the anchoring constraint, proposing board-anchored counts rather than phrase lists. It also volunteered which of its own claims were inferred rather than read, and named the command that would settle each. That is the mode to use from now on; the output-only report card cannot see any of this | kept |

| 156 | (row 155) **The three defects, checked against the board rather than believed.** | Nothing yet — three backlog items, one P1 | **D1 CONFIRMED, and smaller than it looked.** At seed 23 ply 19 `Nxc7+` captures a pawn, gives check and attacks an UNDEFENDED rook on a8; our clause says "capturing their pawn on c7" and the prompt then forbids naming a8. Cause read off the code: `_move_effect` tests `is_capture` first and returns, so the fork branch is unreachable for any capture. Corpus-wide it is 55 turns of 4672 (1.2%) in 11 of 119 runs, model-independent, concentrated in seed 23 at 12.8% — so severe where it lands, not a P1. My first count read 98 until I checked what it flagged and found it scoring "giving check" as understating an incidental pawn. **D3 CONFIRMED.** `with_refutation` opens `if report.refutation_line: return report`, so `_reply_punishes` guards only the reply we compute; an engine-supplied quiet reply bypasses it. `_reply_punishes` returns False for all five replies we shipped (`Kf8` x3, `Ke7`, `Ke6`). 7 of 47 turns rendering a reply. **D2 CONFIRMED and LARGER than the judge claimed.** It cited 2 instances, flagged as inferred; checked properly one of its two is reachable in one move and is not a defect. The corpus says 16 turns carry "your <piece> on <sq> was defended; now it is not" and **0 of the 16 name a square that is actually attacked**, 4 name one the opponent cannot reach at all — including `Rhd1#`, where the coach discusses an undefended pawn on the move that delivers mate. True, unfalsifiable by any check we own, and it teaches a student to defend air | kept |

| 157 | (row 156's D2) **The cause detector now requires a threat to exist, and the measurement that proves it is NOT the one I built first.** | `error_diagnosis` requires the named square to be attacked, or attackable after any one enemy move, before composing a `stopped_defending` cause. Generous on purpose: a piece the opponent is a tempo from winning still earns a warning | **Two counters, and only one of them could have disagreed.** `cause_irrelevant_rate` went 25% -> 0% and is worthless as evidence: it asks the same question the gate asks, so it fell to zero by construction. `cause_materialised_rate` reads the CONTINUATION of the real game, which the detector never sees, and went **62% -> 75%**. The concrete fact is better than either percentage on twelve samples: the gate dropped four warnings, three about threats that never arrived and one that arrived 28 plies later. Of the 12 kept, 10 arrived, median 8 plies. Side effects: causes given 60 -> 57 (not collapsed), spoken turns 81 -> 81, clean unchanged, and only 4 prompts changed across five games. Cost: one turn on seed 17 stopped voicing its cause, and seed 23 ply 13 replaced a concrete-but-useless opener with a vague one, which is the label problem showing through | kept |

| 158 | (row 156's D3, and the first time this process stopped a change) **Every consequence the review attributed to the reply-gate bypass failed to reproduce. The fix was not made.** | Nothing. The backlog item is downgraded to P3 with the four measurements and a statement of what would reopen it | **The bypass is real: `with_refutation` returns early when the engine already supplied a refutation, so `_reply_punishes` guards only the reply we compute, and it returns False for 7 of 47 rendered replies. Everything downstream of that is not.** "An effect is demanded that the supplied line does not carry" is 1 turn of 47, not 7 — 46 reply lines do carry a clause. Quiet replies precede a fall in the student's piece count within 6 plies on 43% of turns against 54% for punishing ones, n=7 against n=35, with no way to attribute the loss to the reply. And the clauses on those replies are not vacuous: 0 of 24 "defending their X on sq" clauses name a square we do not actually attack. So the fix would drop the reply on 7 turns for no measured gain, against row 125's 28-20 pairwise win for supplying it. **Worth recording as the process working rather than as a null result: the measure came first, it said no, and nothing was built** | not built |

| 159 | (B-003) **The coach was already told the recommendation was a repeat, and answered at greater length anyway.** `_achievement_line` has flagged recurrence since v37 ("does the same thing here as it did earlier in the game"). On the sharpest streak — `Nxc7+` recommended on four consecutive turns — the turns ran 47 words then 75, 74, 81 | The stated word budget on those turns is cut to 55% of its tier value, floor 35, plus an instruction naming what to drop: the general principle, the detail of what it wins, the position re-description. The budget is cut and the FACTS are not withheld, because withholding is the documented failure — v37 retired a reason and the model invented three replacements | **Median length ratio (repeat turn / first telling of the same recommendation) 1.23 -> 0.93. Criterion was <= 0.70, so PARTIAL.** Mean 1.15 -> 0.98, mean words 64 -> 55. Both guardrails held at 17/17: the turn still names the move and still closes on a composed lesson. Attribution clean — 18 prompts changed across five games, all of them repeat turns, zero turns where the prompt held and the text moved. **Three measurement errors of my own on the way, all the same shape: the wrong denominator.** I first compared repeat turns to the GLOBAL mean (64.5 against 65.2) and concluded there was nothing to fix; the right comparison is against the first telling of the SAME recommendation. Then I set the accept threshold from the 673-turn corpus median (0.97) and measured v55's 17 turns, whose median is 1.23. What remains: the model does not obey a word count precisely — told 66 words it writes 73 — so `Ke2` is still 1.55x and `Nxc7+` 1.28-1.55x. Closing that means composing the short form ourselves rather than asking for it | kept, criterion missed |

| 160 | (B-003, and a reverted change worth recording) **Scaling the generation ceiling with the word limit did nothing.** I diagnosed the shortfall above as the 300-token ceiling leaving room for the instruction to be ignored, and said so in a code comment | `move_feedback_max_tokens` scaled by the same factor on repeated recommendations. **Reverted** | **The outputs came back BYTE-IDENTICAL. The ceiling was never the binding constraint:** the model writes about 70 words, roughly 100 tokens, and even the reduced ceiling was 165. Median ratio 0.93 -> 0.93, and `run_compare` showed zero prompt changes on three of the five seeds and the only four changed prompts belonging to the OTHER change in the same run. Reverted rather than kept as an unmeasured knob, on this project's own standard that a change with no measured effect goes. Recorded because the reasoning was plausible and wrong, and the comment asserting it was already in the tree | reverted |

| 161 | (B-007, the owner's ruling) **"Do not invent a reason, that's for sure. And I think it'd be better to say nothing if we do not have a justification."** Where no clause could be composed, `_move_achievement` returned the engine's CATEGORY LABEL, and the model can only restate a category: "c5, which improves your pawn structure", "Ke2, which improves king safety by repositioning the king" | One line: return `""` instead of the label. Everything downstream already existed — `_best_move_achievement_line` omits the line rather than leaving a dangling header, and `move_instructions` selects `_REASON_WITHHELD`, which already forbade reasoning one out from the position facts | **Label-only reason lines 4 -> 0, and fabrication did not rise** (turns with a violation 1 -> 1, gating 1 -> 1), which was the real risk: v37's withheld reason produced invention, and the bet was that `_REASON_WITHHELD` — written afterwards — prevents it. It did. **My population figure was wrong three times before I got it right, and each wrong version was more flattering.** "11 of 81" counted turns where `_move_effect` returns nothing, but a reason line is only emitted on comparison tiers. "No square in the line" counted `giving check (rook activity — ...)`, a real clause. "No parenthesis" counted `defending your pawn on d4`, a clause with no label. The truth is 4 of 74 reason lines. Also updated three existing tests that asserted the old label fallback, and added a fourth: without it, a change that silenced the reason on EVERY refinement turn would have passed | kept |

| 162 | **The north star contradicted itself for months and the build followed the wrong half. Every judge score before today was given against the losing reading.** `VISION.md` said end 2 of the bridge is "the specific move or plan", and also that we are NOT the thing that "reports evals and best moves — Lichess and Stockfish already do that". The `grounded-move-advice` spec quoted the first faithfully; `prompts.py` then hardened "or plan" into "name the stronger move" on two severity tiers, which nobody decided | Settled by the owner: **the coaching names the CUE — a square, a piece, a fact — and not the move. A named move appears only behind the hint.** `VISION.md` rewritten with the three levels and the reasoning, the spec amended at the top rather than rewritten, and `_BRIDGE_STANDARD` changed so the judge stops recommending that the coach name a move | **Found by the owner asking why the coach hands over a move at all, which is a product question no metric in this project could have raised.** Three measurements came out of settling it. The coach names a better move on 73 of the spoken turns; on **53 of those the move is still legal on the student's next turn**, so it is prospective advice however it is phrased — `Nxc7+` was handed over on four consecutive turns that way. And on **30 of the 53 the cue IS the move**, because only one legal move reaches the square, which is recorded in VISION as the weak point of the policy rather than smoothed over. The owner's first instinct was to withhold the square too; that was argued down using the project's own rule that chess quality comes from outside — `docs/audit/blind-derivation-a.md` derived "Attention-First Order" blind, and it calls pointing at the cue the only way to get generative work out of a one-shot medium. **Consequence for this ledger: scores before 2026-09-22 are not comparable with scores after it.** A hint button already existed in play mode with the answer sitting in the prose above it | kept |

| 163 | (row 162) **The owner: "all this discussion and you edited docs and 1 script. Seems to me we need to test that change."** `_BRIDGE_STANDARD` was the only functional change in the north-star commit and it shipped untested | Nothing new. An A/B of the rubric text on one saved transcript: `output/coach_review_v54_seed23/transcript.json`, same v2 rubric, same pinned judge agent, only `_BRIDGE_STANDARD` differing — the control arm pulls the pre-`3a21c15` literal out of git with `ast` and asserts the two prompts differ on 20 lines, all of them the edit. `scripts/eval_coach_rejudge.py` was the third caller still passing `--model`, so until it was fixed this test would have been run by `auto` and recorded as opus-5 | **It works. Old standard: "what to remove" lists five items and the named move is not among them, and the top recommendation is a fidelity fix. New standard: "the engine's best move" is item 1, and "delete the best-move clause and make the diagnosis carry the turn" is the highest-leverage change.** Both arms scored 2/10 with pre-gate 5.95 vs 6.0, so the edit moved the critique and left the number alone. The judge also counted the best move in 18 of 20 speaking turns, matching a board-anchored count of the same transcript exactly. **Hazard: the judge can no longer be evidence for B-012.** The rubric now tells it not to recommend naming a move and it duly recommends removing one; that is circular, so B-012's case has to rest on the pairwise harness and the deterministic counters. **My first attempt at this was invalid** — I used the existing v54 review as the control, which was written with rubric **v1** while the new run used v2, so the rubric format was changing alongside the thing under test. Same shape as the three measurement errors in row 159: the wrong comparison group | kept |

| 153 | (row 152) **The fix for the row-148 class of error is a routine, not resolve. The number that would have prevented it was already printed and ignored** — my own output read `prompts 81, identical raw 9, identical after un-mangling 81`, and I then compared all 81 texts | `chess_coach.eval.run_compare` classifies every turn of a pair as SAME (same prompt, same text), MODEL (same prompt, different text — the only honest noise measurement) or INPUT (the prompt changed, so the difference is our code). `eval_repeat_budget.py` DROPS a run that is not a genuine repeat instead of pooling it, `--compare A B` attributes any pair, and `--guard` now prints the exact compare command on failure. 7 tests | **Checked on the three cases that matter.** v53 vs v54 seed 7: "NOT a repeat: 16 of 18 turns changed their PROMPT … only 2 turns are comparable, and 0 of those differ" — the tool refuses in one line the conclusion I spent an hour building. r2 vs r3: "byte-identical on all 18 turns". r1 vs r2: "7 of 18 turns differ with an identical prompt, so that is the model's own non-determinism", naming the plies. The load-bearing test reproduces row 148's shape in miniature — three turns whose prompt and text both changed, one held fixed and identical — and asserts it is not called a repeat. Also silent-on-both-sides turns are excluded from the comparable count, because padding it with turns that were never a measurement is how a moved run looks stable | kept |

| 150 | (row 149's top item, and the owner's ruling on it) **"Coach the good moves" was two of five reviews' highest-leverage change, and 46 of its 53 turns are not a gap.** The owner's ruling: teaching the player about forks when what he just played was a fork is not teaching | Nothing built. The item is closed as a DECISION and the surviving half promoted to its own P1 | **Measured after the ruling, which is what made it safe to close. Of 102 empty turns across the five v54 games: 53 are the engine's top move (silence correct), 44 are non-best moves inside the 1-25cp `equal` band, 5 are opening leniency — and 7 are the student playing a move the coach had ALREADY asked for and got silence for.** Seed 11's own review had agreed with the owner unprompted: "ply 5, 9, 19: student played the engine's top move — silence is fine", so the judges were 2-2-1 and I had reported 2-2. The worked case is seed 23: `Nxc7+` recommended on plies 17, 19, 21 and 23 in four near-identical full-length paragraphs, played on ply 25, met with silence. **The other side of the same missing capability is larger: 17 of 81 spoken turns (21%) re-recommend a move already recommended in that game** — `Nxc7+` x4, `a3` x4, `dxe5` x3, `gxf3` x3. 24 turns of 183 together, against 9 for the lesson mismatch and 11 for the label | kept |

| 151 | (our finding, about row 149's own instrument) **Cross-turn memory has been raised SIX times in BACKLOG.md and the index built a day earlier does not show it.** Lines 2590, 2715, 2783, 2864, 2922 and 3004, under five different priority numbers (#1, sixth, third, then split by cost) | Promoted to an actionable `TOP` heading with the measured 24-of-183, and the gap recorded inside the item | **The index only reads `TOP —` and `OPEN —` headings, so an item living in narrative prose is invisible to it — and the single most-recurring item in a 3000-line file was exactly that.** The instrument was built to stop the cheapest item winning, and on its first real use it still could not see the biggest one. Worth stating because it is the same shape as the defect it was built to fix: a mechanism that only measures what is already in the right format | kept |

| 140 | (rows 137, 139) **The last phrase-matched metric was not just noisy, it was pointing the wrong way.** `top-closer%` read the response's final 14 words for one of five hardcoded cues. It ranked v52 at 22% against v33 at 17% — i.e. we had got worse — when v33 is the documented monoculture peak, and this ledger names the five plies where one lesson closed five of eighteen turns. It also read 0% for v2-v14, which is the cues failing to match that era's wording rather than an absence of repetition | Replaced by `lesson-conc%`, measured from what we COMPOSED: `_composed_lesson` parses the closing instruction out of our own prompt, which is deterministic. Three states were previously mashed together and are now separate columns — **composed** (we named the lesson), **open** (we asked and named nothing, so the MODEL chose the topic) and **silent** (the ladder retired it). `tests/test_hard_metrics_lesson.py`, 5 tests | **Same run, opposite verdict: v33 reads 33% and v52 reads 14%, which matches the record instead of contradicting it.** The arc is now legible across the whole history — v2 had no takeaway instruction at all; v20 was open on 44 of 44 turns, the era when the model picked its own topic and the reviews complained it chose ideas that did not apply; composition arrives at v30 (19%); v33 is the peak at 33%; the ladder lands at v39 (17%); v43 onward holds 14% with reframe on 4 turns and deliberate silence on 2. **And it reports `n/a`, never 0%, when nothing was composed** — a 0 there would read as perfect variety when the truth is the question does not apply, which is the instrument error this ledger keeps rediscovering. The blind spot is named rather than hidden: concentration cannot see `open` turns, so `open` is a column | kept |

| 139 | (row 137) **`cause%` rebuilt as two numbers anchored on the board, and the split is the finding.** GIVEN — did WE compose a cause — is 69-72% of spoken turns and is byte-identical across models, as it must be since the prompt is deterministic. VOICED — did it reach the student — is 92-100%. The old single figure of ~50% was neither, and it hid which half was the problem: **roughly three in ten spoken turns are handed no cause at all, and that is our composition gap, not the model's.** Anchor is the set of squares the composed cause names, never a phrase. Ported across models: 100%/100% qwen against 92%/100% gemma, a spread of ±8 points running in BOTH directions, against the retired metric's consistent 35-point gap. On the older v51 transcript it reads 56%/70%, so real improvement had been happening that the compliance score could not show | `_cause_squares` + `_voices_the_cause` in `scripts/eval_hard_metrics.py`, replacing `_names_a_cause`; the old phrase list kept under `_RETIRED_CAUSE_PHRASES` as a record of what not to repeat. `tests/test_hard_metrics_cause.py`, 6 tests, the load-bearing one asserting a paraphrase scores the same as an echo | **Square extraction has to be ASYMMETRIC and that was found by checking the misses rather than accepting the number.** Strict on the cause, because "Before Bd2 your pawn on b2 was defended" is about b2 and d2 is only where the bishop went. Loose on the coach's text, because a square inside a SAN token is still naming it — at ply 8 the coach answered "the stronger move was Bxc3, capturing their bishop" against a cause about c3, and strict matching called that a miss. Fixing it moved seed 13 from 90% to 100%. **The residual limit is documented rather than closed:** at ply 56 the cause was about e1 and the coach said "moving the rook left it undefended" — correct, no coordinates, unscorable. So the metric UNDERCOUNTS and a figure below 100% reads "at least this many". Closing that gap means matching phrasing again, which is the thing being replaced | kept |

| 137 | (row 60 of BACKLOG, the owner's "I thought we discussed testing multiple models") **First multi-model run: two models x two seeds, and it splits our metrics into two kinds.** The board-checked ones PORT — `clean%` is 100% for both models on both seeds, mind-reading 0, magnitude 0, and endgame teaching is identical at 33%/25% each. The silence gates are identical too (18 and 16 spoken turns per seed, both models), as they should be since they are deterministic. **The phrase-matching ones do not port: `cause%` reads 50%/50% for qwen3:14b and 17%/12% for gemma4:12b-it-qat.** Checked all 29 gemma turns the detector scored zero on — every one of them explains why the move failed. Ply 6: "your pawn on b2 was defended before Bd2 but is no longer protected", which is a better cause statement than our composed wording because it names the before and the after. Ply 18 missed on ONE WORD: `_CAUSE_PHRASES` holds "check that was skipped" and gemma wrote "check was skipped" | Nothing yet — recording the finding. The metric needs relabelling or rebuilding, and broadening the phrase list is the approach rows 27 and 95 already recorded as failing twice | **A metric that matches literal strings taken from our own prompts measures obedience to our phrasing, not quality.** `_CAUSE_PHRASES` is a list of fragments lifted from the composed clauses, so a model that voices our sentences scores high and a model that paraphrases scores near zero while teaching the same thing. Every `cause%` figure in this ledger is therefore partly a qwen3:14b compliance score. The rule that separates the two classes cleanly: check the claim against the BOARD and it ports across models; check it against our own wording and it does not. Second finding, free: the report card runs the coach at `temperature=0.0`, and a qwen re-run was byte-identical to the morning's on 44/44 and 25/25 plies — so there is no sampling noise in these runs at all, and my earlier caveat that they "carry model noise" was wrong | measuring |

| 138 | (row 137) **The provenance block shipped an hour earlier recorded the wrong model, and the multi-model run is what caught it.** `_run_environment` read `config["llm"]`, but `--model` overrides the config, so both gemma arms wrote `model: qwen3:14b` into their own transcripts | Read the values the run ACTUALLY used — `args.model`, `args.base_url`, and a single `_COACH_TEMPERATURE` constant shared with the `Coach` construction so the recorded value cannot drift from the used one. `tests/test_eval_review_environment.py`, 4 tests, pins the config-disagrees-with-argument case. The four transcripts already produced had the block REMOVED rather than corrected by hand — a hand-written provenance block is not provenance, and the model is in the directory name | **Wrong provenance is worse than none.** The block exists to make a confound declare itself, and for one hour it declared the opposite: two arms of a comparison both claiming the same model is exactly the v31/v32 confusion this was built to prevent, with a label saying it could not be happening. Caught within the hour only because the very next thing done with it was a two-model comparison | kept |

| 136 | (rows 133-135) **The metric was driving toward padding, and the product owner said so before the measurement caught it: "I do not think we MUST give a lesson after every turn."** "% of endgame turns voicing an endgame idea" rewards appending a maxim, so I read the five turns it counted as wins instead of the count. Two of the five predate row 134's substitution entirely (a mate turn and a passed-pawn turn already closed on endgame ideas), so the change moved two turns, both on seed 7 — on seed 13 it changed what the prompt GAVE at ply 34 and changed nothing the coach VOICED. Of the two: ply 1000 is a genuine gain, and **ply 58 got worse.** The pre-change turn said Re4 hits the rook on d4 and the bishop on e6, which is true; the post-change turn said "Re4, which cuts the king off", which is false — checked on the board, the black king on e7 has the same five squares (d6 d7 d8 f6 f8) before and after Re4 | **Two fixes.** (1) The substitute must be ANCHORED: `_guidance_takeaway` now takes `guidance_facts` and only considers entries with a verified board fact, so an entry selected merely because the phase matched can no longer close a turn. With no anchored candidate the reframe ("the SAME idea as earlier") stands, which is a complete turn. (2) The prompt's line theme was rendered `moves — theme: X` and read as a claim about move one; it now reads `where the whole line ends up (not a claim about Re4 on its own): X` | **The gain and the regression had a visible difference and it was not the phase tag.** The turn that improved had a board fact behind its entry (a passed pawn on e4 the king could support, which the body itself raised); the turn that got worse had none — "king activity in the endgame" was selected because the position is an endgame, nothing more. So "is this lesson anchored to something we verified here?" separates teaching from padding, and it is the same no-fact-no-claim rule the clause composer already runs on. Second lesson, on instruments: my integration check scored ply 58 as INTEGRATED because the body and the takeaway both contained the word "pawn". A shared common noun is not a shared subject | measuring |

| 103 | **The systemic diagnosis, raised by the product owner: this is a filtering problem, not a content problem.** What the coach says survives NINE sequential filters — path selection (mistake vs asked), the silence gates, tier selection, effect priority (13 patterns, one wins), the lesson ladder, the clause ladder, the fact budget, guidance selection (3 themes from 22), the fidelity gate. Every one was added on its own to answer a specific finding in this very document. None was designed against the others | Nothing yet — this is the diagnosis. Owner: "We never stopped and looked and said maybe it's systemic... I do not want quick fixes" | **The core defect: there is no step that SELECTS, only elimination.** The student reads whatever survived, not what we judged worth saying. That one fact explains most of this session — the same clause five times (a fixed ranking always picks the same winner), new patterns dead on arrival (they enter at the bottom and must beat everything), and a ladder that can rephrase a stale reason but never reach back for a discarded candidate. Proposed shape: **generate** every true thing we could say, then **select** with criteria that can see the whole candidate set and the game so far | open — TOP |
| 104 | (row 103) **Snapshot taken before touching anything.** `scripts/snapshot_clauses.py` + `docs/clause-snapshot.tsv`, enforced by `tests/test_clause_snapshot.py`: 3200 (position, legal move) pairs, byte-identical across runs. Chosen over a design document because the risk here is not "we do not know what to build" — it is that the nine filters encode measured lessons and one could be dropped silently. A document does not protect against that; a diff does | Every legal move in every stored position, not only the moves played — the first attempt used played moves, got 88 rows, and left most branches untouched | Two findings arrived with it. **40% of legal moves produce NO clause at all** (1286 of 3200) — the coverage gap quantified, and what the model is padding around. And **every stored transcript is the same game**: 22 report-card runs contributed 44 distinct positions between them, so every number in this document rests on one game, seed 7. Neither of us had noticed until the dedupe collapsed 22 files into 88 rows | kept |
| 100 | **Nine runs of work are a wash to a blind reader. v33 vs v42: 8-7 to v33, 3 ties, p=1.000.** 16/18 pairs unanimous, so the instrument was behaving — the difference is not there. Consistent with v39 vs v42, also a wash (row 98) | Nothing. This is the readout, and it should redirect the next several changes | What DID change over those nine runs, all verified: falsehoods reaching the student 2-3 per game -> **zero**; turns explaining why a move failed 0% -> **17%**; facts per turn 7.5 -> 0.8. Perceived quality: **unchanged.** Two readings, and this evidence cannot separate them. Either **truthfulness is correctness, not quality** — removing 2-3 lies from 18 turns does not change which paragraph reads better, but it matters enormously to a 1200 who would internalise one for months, in which case the fidelity work was necessary hygiene and expecting it to read as *better coaching* was the error — or we have been optimising truth and structure while a reader reacts to writing quality, specificity, insight and variety, none of which we have touched | kept |
| 101 | (row 100) **The constraint on that conclusion, stated before the run rather than after.** 18 pairs with 3 ties leaves 15 decisive; an exact sign test needs about 12-3 to clear p&lt;0.05 | Nothing — recording the limit | So the finding is "**no LARGE improvement**", not "no improvement". A genuinely useful change winning 60% of comparisons would show as roughly 9-6 and be invisible here. Detecting that needs 3-5 games per arm. Do not read this row as proof the work was worthless; read it as proof the work was not large, and that we cannot currently see moderate | kept |
| 102 | (row 100) **What follows for the plan.** Finishing Diagnosis was next on the list | Demoted. Extending an approach that moves counters but not perceived quality is the weakest available use of time until one of the two readings in row 100 is settled | Proposed instead: the mind-reading fix (row 91), because it is the one queued item a READER notices — a fabricated motive on a move that had none — and so the one thing pairwise could actually detect; then a multi-game arc comparison for sensitivity. Recorded as a proposal, not a decision | open |
| 97 | **BACKLOG item 2 done: pairwise judging validated, and it works.** Same pair (v39 vs v42), same seed, judged twice, 3 votes per pair majority-voted | `scripts/eval_transcript_pairwise.py` — pairs two existing transcripts turn by turn, blind, order randomised, exact two-sided sign test. Reuses the repo's existing pairwise primitives rather than rebuilding them | **Per-ply verdict agreement 17/18 = 94%**, with 16/18 pairs unanimous in BOTH runs and every unanimous verdict identical across runs. The one flip (ply 44) was a 2-1 split both times — genuinely borderline. Against the absolute score's 2.0-5.5 on identical input, this is a different class of instrument. **Adopt it** | kept |
| 98 | (row 97) **What it says about this session's work: a wash.** Run 1 "v42 wins 6-5", run 2 "v39 wins 6-5", p=1.000 in both — the headline reversed only because the true difference is ~zero, and one borderline pair flips a 6-5 | Nothing. This is the readout | v39 and v42 are equivalent to a blind reader. That sits alongside the deterministic counters showing 100% clean turns and cause-naming 0% -> 17% (row 90). Both are true: **measurably more honest and more explanatory, not measurably better to read.** Either three turns in eighteen is too few to shift an impression, or cause-naming is worth less to a reader than the rubric claims | kept |
| 99 | (row 97) **The sensitivity limit, stated before it bites.** 18 pairs with 7 ties leaves ~11 decisive; an exact sign test needs about 10-1 to clear p&lt;0.05 | Nothing — a design constraint to plan around | One game can only detect a LARGE, consistent improvement. Anything of the "a few turns got better" size is invisible to this instrument too, and for those the deterministic counters remain the only thing that will see it. Detecting smaller effects needs 3-5 games per comparison, which at ~15 min per run is one to two hours. Cost per validated A/B is now known, which it was not before | kept |
| 94 | **BACKLOG item 1 done: "winning your rook on e1" is now checked.** v41 ply 60 shipped it. Rf4 is legal and really is check, so legality and the check-marker both passed; the falsehood was the consequence, and it walked past the consequence check only because the sentence said "winning" where `_ATTACK_VERBS` wanted "attacking" | Win/take/capture verbs added to the consequence pattern, which is safe only because it also requires "&lt;piece&gt; on &lt;square&gt;" — bare material idiom ("wins a pawn") names no square and never matches | **Three real falsehoods now caught that six months of runs shipped**: v41 ply 60 "Rf4+, winning your rook on e1" (f4 does not see e1); v40 ply 52 "Rh3+, winning your pawn on d4"; and v20/22/23/24/25/26 ply 36 "bxc4 wins material by capturing the undefended bishop on b4" — verified on the board, bxc4 takes a KNIGHT on c4 and the b4 bishop is untouched. That one shipped in six consecutive runs | kept |
| 95 | (row 94) **Widening the verbs first produced two false positives of our own, found by replaying nine transcripts before trusting the change.** "Bb2 ... misses a chance to capture the pawn on g5" was read as a claim that Bb2 captures on g5; "Kf3 ... but the stronger option was Rxh7, capturing the h7 pawn" was pinned on Kf3. Both sentences are TRUE as written | Two guards. A missed-opportunity phrase before the claim disqualifies it, and the claim window stops at the next token that is unambiguously another move | A checker that flags true sentences is worse than the gap it closes, because it teaches us to ignore it. Also a near-miss worth recording: the window guard was first written with `_SAN_RE`, which matches a bare square — so "hits the h7 pawn" truncated at "h7" and silenced the whole check. Caught by the test suite, and now pinned by a test of its own | kept |
| 96 | **The judge was wrong about the second gap, and building it would have made things worse.** It reported v42 ply 60 as labelling a pin that was not there | Nothing built. Checked first: the black bishop on e6 and the pawn on f7 ARE both pinned, by the rook on g7 against the king on e7. The coach's "the pin on the bishop and pawn to the king is already in place" was true, and precise | A check for it would have flagged a correct statement as a falsehood. **Second time this session a judge per-ply claim failed verification** (the other: v41 ply 34's "mislabelled knight", where the engine had it right and the coach had merely dropped the ownership). The standing rule — treat per-ply claims as leads, verify before acting — earned its keep twice | kept |
| 88 | **The judge's category scores are not a measurement. This retracts a lot of rows below.** Test-retest, asked by the product owner: the SAME v42 transcript, judged three times, returned overall **5.5 / 5.0 / 2.0**, and the fidelity gate fired on one of the three. Dimension disagreement on identical input: Fidelity 5/5/4, Diagnosis 5/4/5, Load Discipline 5/5/4, Stream Behaviour 6/6/5 | Stopped using the score as the target. Added `scripts/eval_hard_metrics.py`: deterministic counters over a transcript, verified to return byte-identical output on three consecutive runs | **The noise on identical input is as large as every effect chased from v37 to v42.** That sequence — 5.4, 5.0, 5.0, 2, 5.3, 5.5 — is indistinguishable from re-rolling dice. Specifically retracted: row 79's "first ungated run in ten" as evidence the authorship rule worked, and v42's "best yet at 5.5". Also note nine runs of near-constant readings: Diagnosis span 1 point across v33-v42, Transfer Handle span 1 | kept |
| 89 | (row 88) What the judge IS good for, on this evidence | Keep it for the written critique and treat every per-ply claim as a lead to check on the board — the standing rule, now with a second reason behind it | Two of this session's real defects came from reading the prose and neither came from the numbers: the move prompt carrying no hanging-piece data at all (row 84), and the fact-budget insight that recovered Load Discipline (row 87's successor). It finds defect classes we had not thought to count. It cannot tell us whether a change helped | kept |
| 90 | **What the hard counters say about nine runs, now that we can measure.** Every column reproducible; `clean%` is grounded in the rules of chess via python-chess, the rest are consistently-applied definitions of ours | Nothing — this is the readout | **Truthfulness: 89/83/89% clean turns for v33-v36, then 100% for v37-v42, six runs.** Nothing the board contradicts has reached the student since v37. **Cause-naming: 0% for v33-v39, then 6%, 17%, 17%** — so the Diagnosis work DID land, three turns a game now explain why the move failed where none did before, and the judge scored Diagnosis 5 through every one of those runs. **Magnitude leakage 0 throughout.** Not improved: repetition, bouncing 6-17% with no trend | kept |
| 91 | (row 90) **A regression I introduced at v37 and never fixed.** Mind-reading openers — "I see you're trying to develop your pieces" — on turns where the student was doing nothing of the kind. Zero in v33-v36, then 2, 2, 2, 1, 1, 1. The judge flagged it in three separate reviews and I did not act | Not yet fixed. It is one turn per game, countable, and on the judge's own "what to remove" list | **open — cheap, do this first next session.** The `clean%` column cannot see it because inventing a motive is not a claim the board can contradict, which is precisely why it needed its own counter | open |
| 92 | **`clean% = 100` overstates, and by how much is known.** It means clean by every check we have | Nothing — recording the limit honestly | The judge still finds one or two per game that our checks miss: v42's fabricated pin at ply 60, v41's "Rf4+, winning your rook on e1" (a rook on f4 does not attack e1 — the phrase escapes `_ATTACK_TARGET_RE` because "winning" is not in `_ATTACK_VERBS`). Each confirmed one becomes a permanent check; that is how the checks went from 3 to 8 this session, and it is cumulative in a way the score never was | open |
| 93 | **Reproducibility hole in the ledger itself.** `config.yaml` carries UNCOMMITTED changes that every run from v33 to v42 used: `model: qwen3:8b -> qwen3:14b` and `guidance: off -> on`. A fresh checkout reproduces none of these numbers | Flagged to the product owner; not committed without a decision, since it changes the project's declared defaults | **open.** Every measurement in this document is conditional on two settings that are not in version control | open |
