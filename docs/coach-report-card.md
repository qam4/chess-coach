...
| 148 | (rows 143-147) **v54: the five-seed sweep re-run on the current build, and it was a controlled test by accident.** All 81 prompts came back byte-identical to v53 once v53's mojibake is normalised to a real em dash — so the only input that changed across the two sweeps was the encoding fix | Nothing. A measurement | **Every counter computed from our own prompt is identical on all five seeds** — `spoke`, `plies`, `cause_given_pct`, `lesson_conc_pct` (10-12%), `lesson_open_pct`. So the encoding fix, which removed corrupt bytes from 72 of 81 prompts, moved nothing we track. **And the number that matters more: 32 of 81 spoken turns are byte-identical between two runs of the same seed, same model, temperature 0 — mean character similarity 0.70-0.83.** Sixty percent of the coach's prose is not reproducible. The cautionary case is seed 17's `clean_pct` 89 -> 95, which I nearly credited to the confinement fix: it is one turn, ply 65, whose prompt was byte-identical in both runs and where the model simply said something else. A one-turn difference is inside the churn, which retrospectively bounds every model-side number in this ledger measured on one run per arm. It also makes `--guard` unsound as built — it failed on `cause_voiced_pct` at -8pp against a 3pp tolerance, on the metric row 141 already recorded as swinging 50 points and "never a coaching signal" | kept |
