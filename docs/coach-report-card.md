...
| 42 | (v29, rubric v2) The gate fired on two classes **already on the backlog since v24/v26**, not a new one: mate called "a check" (ply 1003) and intent the coach cannot know (ply 38, "aimed to develop your king's bishop" with no bishop on the board) | Terminal-move check added, gating the send path, plus a mate branch in the composed fallback | Catches ply 1003 in **all four** stored runs (v26-v29), zero false positives over ~176 turns. Intent check still open | half done |
